export const en = {
  name: {
    first: 'Andrey',
    last: 'Polyakov',
  },

  nav: {
    about: 'About',
    projects: 'Projects',
    skills: 'Skills',
    experience: 'Experience',
    whyChooseMe: 'Why Me',
    contact: 'Contact',
  },

  hero: {
    title: 'Full Stack QA Engineer',
    contactBtn: 'Contact Me',
    downloadCV: 'Download CV',
    available: 'Open to opportunities',
    scrollDown: 'Scroll down',
    subtitle: 'Building reliable autotests and ensuring product quality',
    roles: [
      'Test Architect',
      'Quality Guardian',
      'Bug Hunter',
    ],
  },


  about: {
    title: 'About Me',
    description:
      "Hi, I'm Andrey — a Fullstack QA Engineer with 3+ years of experience ensuring the quality and reliability of modern web applications. I specialize in Python, Playwright, and Pytest, focusing on building robust automated test frameworks and delivering seamless user experiences.",
    socialLinks: '',
  },


  projects: {
    title: 'My Projects',
    inProgress: 'Soon',
    items: {
      qaDesktop: {
        title: 'QA Desktop',
        description: 'Team links hub',
      },
      pytestFramework: {
        title: 'API HTTPX Template',
        description: 'Pytest + Allure + CI/CD',
      },
      playwrightTemplate: {
        title: 'Playwright Template',
        description: 'E2E + Page Object',
      },
    },
  },


  skills: {
    title: 'Skills',
  },

  // Stats
  stats: {
    title: 'Statistics',
    subtitle: 'Numbers that speak for themselves',
    years: 'Years of Experience',
    tests: 'Autotests Written',
    bugs: 'Bugs Found',
    coffee: 'Cups of Coffee',
  },


  experience: {
    title: 'Experience',
    jobs: [
      {
        title: 'Fullstack QA Engineer',
        company: 'Leads.tech',
        period: 'Feb 2024 — Present',
        points: [
          'UI & API test automation with Python (Pytest, Playwright) and Allure reports',
          'CI/CD integration with GitLab and Allure TestOps, flaky test monitoring',
          'Acting QA Lead: coordinating a team of 4 QA engineers',
          'Mentoring manual QA and code review of autotests',
        ],
      },
      {
        title: 'QA Engineer',
        company: 'IT Finance',
        period: 'Sep 2023 — Feb 2024',
        points: [
          'Load testing of highload systems (JMeter, Locust)',
          'Functional and integration testing of monolithic architecture (API, UI)',
          'Database work (PostgreSQL, ClickHouse, Redis), log and metrics analysis (Kibana, Grafana)',
          'API documentation design and maintenance (Swagger / OpenAPI)',
        ],
      },
      {
        title: 'QA Engineer',
        company: 'Легко учиться',
        period: 'Apr 2023 — Aug 2023',
        points: [
          'Functional and integration testing of web application',
          'API testing and database work',
          'UI automation (Java, Selenium, Page Object)',
        ],
      },
    ],
  },

  // Testimonials
  testimonials: {
    title: 'Testimonials',
    subtitle: 'What colleagues say about working with me',
    items: [
      {
        name: 'Alex Smirnov',
        role: 'Tech Lead',
        company: 'Leads.tech',
        text: 'Andrey is a true professional. His autotests saved us hundreds of hours of manual testing. Highly recommended!',
        rating: 5,
      },
      {
        name: 'Maria Ivanova',
        role: 'Product Manager',
        company: 'IT Finance',
        text: 'Excellent QA engineer with deep understanding of development processes. Always finds critical bugs before release.',
        rating: 5,
      },
      {
        name: 'Dmitry Kozlov',
        role: 'Senior Developer',
        company: 'Leads.tech',
        text: 'Working with Andrey always means quality results. His tests are stable and cover all edge cases.',
        rating: 5,
      },
    ],
  },


  whyChooseMe: {
    title: 'Why Choose Me',
    features: {
      experience: {
        title: '3+ Years in QA',
        description: 'Experience at all stages — from startup to highload production.',
      },
      automation: {
        title: 'Automation',
        description: 'UI and API autotests that reduce regression time.',
      },
      quality: {
        title: 'Full QA Cycle',
        description: 'From requirements analysis to post-release support.',
      },
      reliable: {
        title: 'Stable Tests',
        description: 'No flaky tests — only reproducible results.',
      },
      teamwork: {
        title: 'Teamwork',
        description: 'Effective collaboration with dev, DevOps and management.',
      },
      remote: {
        title: 'Remote Work',
        description: 'Self-organization, async communication, responsibility.',
      },
    },
  },

  // Contact
  contact: {
    title: 'Contact Me',
    form: {
      name: 'Name',
      email: 'Email Address',
      message: 'Message',
      submit: 'Submit',
      sending: 'Please wait',
      success: 'Message sent successfully!',
      errors: {
        nameRequired: 'Name is required',
        nameShort: 'Name is too short',
        emailRequired: 'Email is required',
        emailInvalid: 'Invalid email',
        messageRequired: 'Message is required',
        messageShort: 'Message is too short (min 10 chars)',
      },
    },
  },

  // Footer
  footer: {
    copyright: 'Copyright © {year} Andrey Polyakov. All rights reserved.',
    description: 'Full Stack QA Engineer building reliable autotests and ensuring product quality.',
    quickLinks: 'Quick Links',
    contact: 'Contact',
    backToTop: 'Back to Top',
    madeWith: 'Made with',
    using: 'using Next.js & Tailwind CSS',
  },


  chat: {
    title: 'AI Assistant',
    subtitle: 'Ask me anything about Andrey',
    placeholder: 'Start a conversation...',
    inputPlaceholder: 'Type your message...',
    open: 'Open chat',
    close: 'Close chat',
    error: 'Sorry, something went wrong. Please try again.',
  },
};
